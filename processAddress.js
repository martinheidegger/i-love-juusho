...
module.exports = function (fields) {
  return function processAddress (field) {
    ...
    if ((res = /(([０-９]+)・)?([０-９]+)[Ｆ階]?・([０-９]+)[Ｆ階]$/ig.exec(rest))) {
      if (res[2]) {
    ...
    if ((res = /([０-９]+)(([^０-９番地号丁目]*[一二三四五六七八九十]+[^０-９]*)+)$/.exec(rest))) {
      buildingName = res[2]
    ...
    if ((res = /([０-９]+)街区(([０-９]+)外?号?)?$/ig.exec(rest))) {
      areaName = rest.substr(0, res.index)
      area = res[1]
      district = res[3]
      rest = ''
    }
    ...
    if ((res = /^([^０-９]+)([０-９]+)街区([０-９]+)(画地|番)?$/.exec(rest))) {
      areaName = res[1]
      area = res[2]
      district = res[3]
      rest = ''
    ...
    if ((res = /^([０-９]+)号([０-９]+)－([０-９]+)$/ig.exec(rest))) {
      area = res[1]
      district = res[2]
      buildingNumber = res[3]
      rest = ''
    ...
    if ((res = /((第[０-９]+)?([^０-９－丁番地街区号]+)ビル(ディング)?)$/ig.exec(rest))) {
      buildingName = res[1]
      rest = rest.substr(0, res.index)
    ...
    if ((res = /^([０-９]+)組(－([０-９]+))?(－([０-９]+))?$/ig.exec(rest))) {
      area = res[1]
      district = res[3]
      buildingNumber = res[5]
      rest = ''
    }

    var district
    if ((res = /([０-９]+)番[耕山]?地?丙?町?([－の]?([０-９]+)番?)?([－の]?([０-９]+))?[－の]?([^０-９－丁番地街区条号]+)?$/ig.exec(rest))) {
      if (res[3]) {
        if (buildingNumber) {
          area = res[1]
          district = res[3]
